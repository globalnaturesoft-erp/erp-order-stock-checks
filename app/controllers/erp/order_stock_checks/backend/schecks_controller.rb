        ...
        def new
          ...
          @order.order_details.each do |ord|
            @scheck.scheck_details.build(
              ...
              available: (ord.quantity <= ord.product.get_stock(warehouse_ids: ord.warehouse_id) ? true : false)
            )
          end
        ...
        private
          ...
          def scheck_params
            params.fetch(:scheck, {}).permit(:code, :employee_id, :order_id,
                          :scheck_details_attributes => [ :id, :order_detail_id, :available, :serials, {:alternative_items => {}}, :note, :_destroy ])
          end
      end
    end
  end
